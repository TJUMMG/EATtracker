...
##im-原图，pos-目标位置，model_sz-希望输出大小，original_sz-原图上裁剪大小，avg_chans-通道平均值
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='torch'):
    ...
    crop_info['crop_cords'] = [context_xmin, context_xmax, context_ymin, context_ymax]##crop出来的图像在原图上面的两点坐标
    crop_info['empty_mask'] = tete_im##返回原图经过通道均值pad后的大小所对应的全0mask
    crop_info['pad_info'] = [top_pad, left_pad, r, c]##图像pad信息和原图大小
...
def aug_apply(bbox, param, shape, inv=False, rd=False):##平移和尺度变化是对图片来说的，用对裁剪图片的框进行平移和尺度变化来实现
    """
    apply augmentation
    ...
    if not inv:
        ...
        if 'scale' in param:
            ...
            scale_x = min(scale_x, float(imw) / w)##防止尺度变换后的范围会超过51*511图片本身
        ...
        if 'shift' in param:
            ...
            bbox = Corner(x1 + tx, y1 + ty, x2 + tx, y2 + ty)##平移是对所有坐标量进行加的操作
...
def create_logger(cfg, modelFlag='OCEAN', phase='train'):
    ...
    final_output_dir = root_output_dir / model##存放model的文件路径
    ...
    final_log_file = final_output_dir / log_file##存放log的文件路径
    head = '%(asctime)-15s %(message)s'##代表行时间，日志内容;这样输出内容便是这二者组合的内容了
    logging.basicConfig(filename=str(final_log_file),##进行日志文件的基本配置  filename为日志输出的文件名，如果指定这个此信息后，实际上会启用FileHandler,而不再是StreamHandler,这样日志信息便会输出到文件中了
                        format=head)##format指定日志信息的输出格式
    logger = logging.getLogger()##声明了一个logger对象，日志输出的主类
    logger.setLevel(logging.INFO)##设置日志的等级，即只能输出包含以及大于INFO级别的信息
...
def _build_lr_scheduler(optimizer, config, epochs=50, last_epoch=-1):
    return LRs[config.TYPE](optimizer, last_epoch=last_epoch,##WARMUP-step, LR-log
                            epochs=epochs, **config.KWARGS)


def _build_warm_up_scheduler(optimizer, cfg, epochs=50, last_epoch=-1, modelFLAG='OCEAN'):
    #cfg = cfg[modelFLAG]
    warmup_epoch = cfg.TRAIN.WARMUP.EPOCH##5
...
def build_lr_scheduler(optimizer, cfg, epochs=50, last_epoch=-1, modelFLAG='OCEAN'):
    cfg = cfg[modelFLAG]
    if cfg.TRAIN.WARMUP.IFNOT:##执行
        return _build_warm_up_scheduler(optimizer, cfg, epochs, last_epoch)
    else:
        return _build_lr_scheduler(optimizer, cfg.TRAIN.LR, epochs, last_epoch)##不想执行warmup可以执行这个，并且可以更改学习率的下将走向:log,step,multistep,linearstep,cosstep
