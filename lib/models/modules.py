...
# Modified ResNet
# ---------------------
class ResNet_plus2(nn.Module):##(Bottleneck--ResNet里面的每组卷积里面的基本组成沙漏状模块, [3, 4, 6, 3]--每组卷积里面沙漏模块个数, used_layers=[3]--使用倒数第一块还是第二块卷积组, online=F)
    def __init__(self, block, layers, used_layers, online=False):
        ...
        if self.layer3_use:
            if online:
                ...
                self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2)##conv4_x
        ...
        if self.layer4_use:
            self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=4)  # 7x7, 3x3  ##conv5_x
