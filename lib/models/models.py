...
class Ocean(Ocean_):
    def __init__(self, align=False, online=False):
        ...
        self.features = ResNet50(used_layers=[3], online=online)   # in param  ##返回 x_stages=[conv1, conv2_x, conv3_x], x=conv4_x
        self.neck = AdjustLayer(in_channels=1024, out_channels=256)  ##搜索帧返回-31*31*256, 模板帧返回-7*7*256
        self.connect_model = box_tower(inchannels=256, outchannels=256, towernum=4)  ##回归输出-25*25*4, 分类输出-25*25*1, 分类分支特征融合输出-25*25*256, 回归分支四层卷积后输出25*25*256
...
class OceanPlus(OceanPlus_):
    def __init__(self, online=False, mms=False):
        ...
        self.features = ResNet50(used_layers=[3], online=online)   # in param   ##ResNet50网络的前4块，同Ocean
        self.neck = AdjustLayer(in_channels=1024, out_channels=256)
