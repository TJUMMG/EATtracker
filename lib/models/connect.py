...
# from lib.models.dcn.deform_conv import*
...
class AdjustLayer(nn.Module):##  backbone网络后的调整通道的调整层
    ...
    def forward(self, x, crop=False):
        ...
        if x_ori.size(3) < 20 and crop:
            l = 4
            r = -4
            xf = x_ori[:, :, l:r, l:r]  ##模板帧feature map的原始大小为15*15,从中心位置crop后的大小为7*7

        if not crop:  ##crop标志是否为模板帧
            return x_ori  ##搜索帧返回--原大小
        else:
            return x_ori, xf  ##模板帧返回，但只使用xf--只取中心7*7
...
class box_tower(nn.Module):  ##Ocean里买的多尺度特征融合，分类分支，回归分支
    """
    box tower for FCOS reg
    """
    def __init__(self, inchannels=512, outchannels=256, towernum=1):##inchannels=256, outchannels=256, towernum=4
        super(box_tower, self).__init__()
        tower = []
        cls_tower = []
        # encode backbone
        self.cls_encode = matrix(in_channels=inchannels, out_channels=outchannels)##分类分支不同步长的空洞卷积组
        self.reg_encode = matrix(in_channels=inchannels, out_channels=outchannels)
        self.cls_dw = GroupDW(in_channels=inchannels)##分类分支特征融合部分的相关操作和相加
        self.reg_dw = GroupDW(in_channels=inchannels)
    ...
    def forward(self, search, kernal, update=None):
        ...
        # cls and reg DW
        cls_dw = self.cls_dw(cls_z, cls_x)##25*25*256
        reg_dw = self.reg_dw(reg_z, reg_x)##2525*256
        x_reg = self.bbox_tower(reg_dw)
        x = self.adjust * self.bbox_pred(x_reg) + self.bias
        x = torch.exp(x)##25*25*4

        # cls tower
        c = self.cls_tower(cls_dw)
        cls = 0.1 * self.cls_pred(c)##25*25*1

        return x, cls, cls_dw, x_reg  ##回归输出-25*25*4, 分类输出-25*25*1, 分类分支特征融合输出-25*25*256, 回归分支四层卷积后输出25*25*256
