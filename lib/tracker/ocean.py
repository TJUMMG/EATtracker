...
class Ocean(object):
    ...
    def init(self, im, target_pos, target_sz, model, hp=None):
        ...
        return state
    ##target_pos:原图位置，target_sz=target_sz*scale_z:在search image[255,255]上面的大小，scale_z=127/s_z
    def update(self, net, x_crops, target_pos, target_sz, window, scale_z, p):
        ...
        # bbox to real predict ##得到score map上面每个点对应到search image[255,255]上面的bbox
    ...
    def track(self, state, im, online_score=None, gt=None):
        p = state['p']##超参数
        net = state['net']##网络模型
        avg_chans = state['avg_chans']##图片通道均值
        window = state['window']##score map的余弦窗
        target_pos = state['target_pos']##在原图上的位置
        target_sz = state['target_sz']##在原图上的大小
